...
import time
"""
pandas → For data manipulation and analysis (used for reading, grouping, merging, etc.).

matplotlib.pyplot (aliased as mpl) → For creating visualizations like line plots.

time → For adding pauses (sleep) between plot generations to avoid overlap.
"""


def Data_Processing(State_Names_noted):
    """
    Defines a function named Data_Processing that takes one parameter:

    State_Names_noted: a DataFrame containing all raw data for a single state.
    """
    print(State_Names_noted.head(10))
    """
    Displays the first 10 rows to confirm that the input data is loaded correctly.
    """
   

    #State_Names_noted.to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Data.xlsx", index=False)
    ...
    """
    Groups data by each Year-Month pair.

    Calculates:

        Mean of maximum and minimum temperatures.
    ...
    """
    Converts monthly means into yearly means by averaging across months.
    """
    ...
    """
    Combines both temperature tables into one.
    ...
    """
    Combines max & min temperature yearly averages.
    ...
    """
    Converts monthly total into yearly total by adding across months.
    """
    ...
    """
    Adds average monthly and yearly reservoir levels.
    """
    ...
    """
    Combines both level and monthly tables into one.
    ...
    """
    Combines Levels yearly averages.
    ...
    Monthly.to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Monthly_Data.xlsx", index=False)
    """"
    ...
    Yearly.to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Yearly_Data.xlsx", index=False)
    """"
    Save Data in EXCEL file 
    """
    ...
    """
    Combines Month and Year into a single text column like 5-2010.
    """
    ...
    mpl.grid(True)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Total_Rainfall.png",dpi = 300)
    ...
    mpl.grid(True)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Maximum_Temperature_(yearly).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.grid(True)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Minimum_Temperature_(yearly).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.xticks(Yearly["Year"], rotation=45, ha='right')
    mpl.title(f"Yearly Yield of {state_name}")
    mpl.xlabel("<-------Year-------->")
    mpl.ylabel("<-------Yield------->")
    mpl.grid(True)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Monthly_Average_Yield.png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    Plots one variable (Rainfall, Max Temp, Min Temp, Yield) vs Year.

    Adds titles, labels, grid, saves to PNG, and pauses briefly to show.
    """
    
    years_present = sorted(State_Names_noted["Year"].unique())
    ...
    """
    Ensures the dataset includes all years (2000–2022).

    Prints a warning if middle years are missing
    """
    ...
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Classical_Decompose(Original).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.xticks(Month_and_Year["Month-Year"][::12],rotation=45)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Classical_Decompose(Trend).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.xticks(Month_and_Year["Month-Year"][::12],rotation=45)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Classical_Decompose(Seasonal).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.xticks(Month_and_Year["Month-Year"][::12],rotation=45)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Classical_Decompose(Residual).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    Each component is then plotted and saved separately.
    """
    ...
    """
    A more advanced decomposition (robust to outliers).
    """
    ...
    mpl.xticks(Month_and_Year["Month-Year"][::12],rotation=45)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_STL_Decompose(Original).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.xticks(Month_and_Year["Month-Year"][::12],rotation=45)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_STL_Decompose(Trend).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.xticks(Month_and_Year["Month-Year"][::12],rotation=45)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_STL_Decompose(Seasonal).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.plot(Month_and_Year["Month-Year"],result.resid, label ="Redidual",color ="r")
    mpl.legend(loc ="upper left")
    mpl.title(f"STL Decompose of {state_name}")
    mpl.xlabel("<-------Month-Year------->")
    mpl.ylabel("<-------Monthly_Yield------->")
    ...
    mpl.xticks(Month_and_Year["Month-Year"][::12],rotation=45)
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_STL_Decompose(Redidual).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    print("Outlier rows:\n", outliers)
    outliers.to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_outliers.xlsx", index=False)
    ...
    print(Monthly_clean)
    Monthly_clean.to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Monthly_clean_Data.xlsx", index=False)
    ...
    """
    Uses last two years as the test set.
    ...
    """
    Builds a SARIMAX model to forecast yield.

    Includes exogenous variables: rainfall, temperature, and level.
    """
    ...
    doc.save(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Summary.docx")
    """
    Saves the model summary in a Word document with borders.
    """

    forecast = results.predict(start=len(y_train), end=len(y_train) + len(y_test) - 1, exog=X_test)
    forecast = forecast.reset_index(drop=True)
    """
    Predicts test period yields.
    """
    ...
    print(compare_df)
    compare_df.to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_compare.xlsx", index=False)
    ...
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Residuals_over_Time.png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Residuals_Distribution.png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_ACF_and_PACF_of_residual.png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    """
    Metric	                    Meaning
    MAE	            Average absolute error (in same units as yield)
    MSE	            Penalizes large errors (squared)
    MAPE	        Mean % error, scale-free
    """
    ...
    print(exog_2023)
    exog_2023.to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_exog_2023.xlsx", index=True)

    # ✅ Extend exogenous data pattern for 2024 (copied or slightly varied from 2023)
    exog_2024 = exog_2023.copy()

    # Keep same monthly pattern as 2023
    ...
    print(exog_2024)
    exog_2024.to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_exog_2024.xlsx", index=True)
    ...
    forecast_2023_df[["Year", "Month", "Forecasted_Monthly_Yield"]].to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_forecast_2023.xlsx", index=False)

    print("\n📈 Forecasted Monthly Yield for 2024:")
    print(forecast_2024_df[["Year", "Month", "Forecasted_Monthly_Yield"]].to_string(index=False))
    forecast_2024_df[["Year", "Month", "Forecasted_Monthly_Yield"]].to_excel(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_forecast_2024.xlsx", index=False)
    ...
    mpl.legend()
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Forecasted_Monthly_Yield_for_2023_(95%_Confidence_Interval).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
    ...
    mpl.legend()
    mpl.tight_layout()
    mpl.savefig(f"D:\\Internship 2025\\My ISI Project\\Main\\{state_name}_Forecasted_Monthly_Yield_for_2024_(95%_Confidence_Interval).png",dpi = 300)
    mpl.show(block=False)
    mpl.pause(0.1)
    time.sleep(2)           
    mpl.close()
...
Main_Data = pd.read_csv("D:\\Internship 2025\\ISI Project\\merged_gram_reservoir.csv")
